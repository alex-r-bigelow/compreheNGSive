'''
This file is a modified version of durus 3.9's btree.py, changed to allow duplicate keys

Modifications contributed by Alex Bigelow, Oct 2012
'''

from durus.persistent import PersistentObject

class duplicateList(list):
    def __getslice__(self, low, high, step=None):
        if step != None:
            return duplicateList(list.__getslice__(self,low,high,step))
        else:
            return duplicateList(list.__getslice__(self,low,high))
    def __getitem__(self, key):
        if isinstance(key,slice):
            return duplicateList(list.__getitem__(self,key))
        else:
...
class BNode (PersistentObject):
    ...
    def insert_item(self, item):
        """(item:[key:anything, value1:anything, value2:anything, ...])
        """
        assert not self.is_full()
        key = item[0]
        position = self.get_position(key)
        if position < len(self.items) and self.items[position][0] == key:
            self.localCount -= len(self.items[position])-1
            self.globalCount -= len(self.items[position])-1
            
            self.items[position] = item
            
            self.localCount += len(item)-1
            self.globalCount += len(item)-1
            ...
            if child.is_full():
                ...
                if key == self.items[position][0]:
                    ...
                    self.globalCount -= self.nodes[position].globalCount
    ...
    def split_child(self, position, child):
        ...
        self.localCount += len(splitting_key)-1
        bigger.updateCounts()
        child.localCount -= len(splitting_key)-1 + bigger.localCount
        child.globalCount -= len(splitting_key)-1 + bigger.globalCount
    ...
    def updateCounts(self):
        ...
        if not self.is_leaf():
            for node in self.nodes:
                self.globalCount += node.globalCount
    ...
    def delete(self, key):
        ...
        """
        raise Exception("For now, the deleting from the BTree has been disabled")
        """def is_big(node):
    ...
    def count_range(self, start=None, end=None, closed_start=True, closed_end=False):
        """() -> int
        How many actual values (not items) are between the
        supplied key values?
        """
        total = self.globalCount
        if start != None:
            # shave off stuff that is less than (or equal to, if not closed_start) start
            position = self.get_position(start)
            if self.is_leaf():
                for item in self.items[:position]:
                    total -= len(item)-1
                if not closed_start:    # also shave off the endpoint
                    total -= len(self.items[position])-1
            else:
                for node in self.nodes[:position]:
                    total -= node.globalCount
                # we want to subtract off stuff that DOESN'T fit our criteria, or the inverse
                total -= self.nodes[position].count_range(start=None,end=start, closed_start=True, closed_end=not closed_start)
        if end != None:
            # shave off stuff that is less than (or equal to, if not closed_start) start
            position = self.get_position(end)
            if self.is_leaf():
                for item in self.items[position+1:]:
                    total -= len(item)-1
                if not closed_end:    # also shave off the endpoint
                    total -= len(self.items[position])-1
            else:
                for node in self.nodes[position+1:]:
                    total -= node.globalCount
                # we want to subtract off stuff that DOESN'T fit our criteria, or the inverse
                total -= self.nodes[position].count_range(start=end,end=None, closed_start=not closed_end, closed_end=True)
        return total
...
class BTree (PersistentObject):
    ...
    class sliceIterator:
        def __init__(self, rootObj, sliceObj):
            self.rootObj = rootObj
            self.start = sliceObj.start
            self.end = sliceObj.stop
            self.step = sliceObj.step
        def __iter__(self):
            skips = 0
            for item in self.rootObj.iter_from(self.start):
                if item[0] >= self.end:
                    break
                if skips == 0:
                    yield item[1:]
                ...
                if skips >= self.step:
                    skips = 0
        def __repr__(self):
            return repr([repr(i) for i in self])
    
    def __getitem__(self, key):
        """
        Unlike a normal dictionary, this one supports slicing as there's an inherent internal order.
        A normal lookup will return a list of values that have been added for the key (or KeyError if none have been added). This list
        will be sorted by the order in which they were first added.
        A slice operation [start:end:step] will return a list of every step-th of these lists from key start (inclusive) to end (exclusive).
        """
        if isinstance(key,slice):
            return BTree.sliceIterator(self.root,key)
        else:
            item = self.root.search(key)
            if item is None:
                raise KeyError(key)
    ...
    def add(self, key, value=True):
        ...
        Add value to self[key].
        """
        if isinstance(value,duplicateList):
            raise Exception('Attempted to reinsert result list as a value into BTree... you should add the items individually (this would create a list inside a list)')
        if self.root.is_full():
            ...
            self.root = node
        newItem = self.root.search(key)
        if newItem == None:
            newItem = duplicateList([key,value])
        else:
            if value not in newItem[1:]:
    ...
    def count_range(self, start, end, closed_start=True, closed_end=False):
        return self.root.count_range(start,end,closed_start,closed_end)
